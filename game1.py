...
class MonsterGameGUI:
    ...
    def create_game_board(self):
        ...
        self.clear_board()  # Clear the previous game board
        ...
        for i in range(1, num_boxes + 1):
            btn = tk.Button(self.board_frame, text=f"Goa {i}", width=10, height=3,
                            command=lambda i=i: self.check_guess(i), bg="lightblue")
    ...
    def check_guess(self, guess):
        ...
        if guess == self.enemy_position:
            ...
            self.buttons[guess - 1].config(bg="green")  # Change button color to green when correct
            ...
            self.buttons[guess - 1].config(bg="red")  # Change button color to red when wrong
            if self.lives == 0:
                self.game_over()
    ...
    def show_message(self, message, msg_type):
        ...
        self.root.after(1500, self.create_game_board)  # Refresh the game after 1.5 seconds
